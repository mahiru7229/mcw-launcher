...
def test_gui_uses_mcw_core_boundary_instead_of_src_core() -> None:
    violations: list[str] = []
    paths = list((PROJECT_ROOT / "src" / "gui").rglob("*.py")) + [PROJECT_ROOT / "launcher.py"]
    for path in paths:
        for module in _imports(path):
...
def test_launcher_public_api_modules_exist() -> None:
    missing: list[str] = []
    paths = list((PROJECT_ROOT / "src" / "gui").rglob("*.py")) + [PROJECT_ROOT / "launcher.py"]
    for path in paths:
        for module in _imports(path):
            if not module.startswith("mcw_core.api."):
                continue
            relative = module.removeprefix("mcw_core.api.")
            target = PROJECT_ROOT / "mcw_core" / "api" / Path(*relative.split("."))
