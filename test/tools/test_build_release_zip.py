...
def test_validate_release_version_accepts_current_beta() -> None:
    assert validate_release_version("v0.6.0-beta.2") == "0.6.0-beta.2"
...
def test_build_release_zip_writes_update_manifest(tmp_path: Path) -> None:
    ...
    output = project / "release" / "MCW-Launcher-v0.6.0-beta.2-windows-x64.zip"

    build_release_zip(project, executable, "0.6.0-beta.2", output)

    with zipfile.ZipFile(output) as archive:
        root = "MCW-Launcher-v0.6.0-beta.2-windows-x64"
        manifest = json.loads(archive.read(f"{root}/mcw-update.json"))
        assert manifest["version"] == "0.6.0-beta.2"
...
def test_release_script_runs_directly_from_any_working_directory(tmp_path: Path) -> None:
    ...
    executable = tmp_path / "MCW Launcher.exe"
    executable.write_bytes(b"fake-exe")
    output = tmp_path / "MCW-Launcher-v0.6.0-beta.2-windows-x64.zip"

    result = subprocess.run([sys.executable, str(project_root / "tools" / "build_release_zip.py"), "--exe", str(executable), "--version", "0.6.0-beta.2", "--output", str(output)], cwd=tmp_path, capture_output=True, text=True, timeout=30)
