...
class FTBPackInstaller:
    ...
    @staticmethod
    def install(project_id: int, version_id: int, instance_name: str, install_optional_files: bool = True, allowed_release_types: tuple[str, ...] | list[str] | set[str] | None = None, reporter: ProgressReporter | None = None) -> FTBModpackInstallResult:
        ...
        staging = Paths.ftb_root() / "staging" / uuid.uuid4().hex
        staging.mkdir(parents=True, exist_ok=False)
        instance = None
        try:
            FTBPackInstaller._download_files(project.project_id, version.version_id, selected, staging, reporter)
            download_pause_controller.raise_if_requested()
            if reporter is not None:
                reporter.status(ProgressStage.INSTALLING_MOD_LOADER, f"Preparing {loader_name.title()} {loader_version}...")
            ...
            ModLoaderManager.prepare(minecraft, *resolved_loader, reporter=reporter)
            download_pause_controller.raise_if_requested()
            instance = InstanceManager.create(name=name, version=minecraft, mod_loader=resolved_loader)
            try:
                FTBPackInstaller._copy_staging(staging, Path(instance.instance_dir), reporter)
        ...
        if instance is None:
            raise RuntimeError("FTB modpack installation did not create an instance.")
        if reporter is not None:
            reporter.succeeded(ProgressStage.FINISHED, f"FTB modpack '{project.name}' is ready.")
    ...
    @staticmethod
    def _download_files(project_id: int, version_id: int, files: tuple[FTBFile, ...], staging: Path, reporter: ProgressReporter | None) -> None:
        total = len(files)
        if reporter is not None:
            reporter.files(ProgressStage.CHECKING_MODPACK, "Preparing FTB modpack files...", 0, total)
        for index, file in enumerate(files, start=1):
            download_pause_controller.raise_if_requested()
            relative = FTBPackInstaller._file_relative_path(file)
            destination, _ = ArtifactDownloadService.safe_destination(staging, relative.as_posix())
            request = ArtifactRequest(
                ...
                max_attempts=3,
                max_bytes=max(0, file.size) or 2 * 1024 * 1024 * 1024,
            )
            artifact_download_service.download(
                request,
                reporter=reporter,
                progress_stage=ProgressStage.DOWNLOADING_MODPACK,
                progress_message=f"Downloading FTB file {index}/{total}: {file.name}",
            )
            if reporter is not None:
                reporter.files(ProgressStage.CHECKING_MODPACK, "Preparing FTB modpack files...", index, total)

    @staticmethod
    def _copy_staging(staging: Path, destination: Path, reporter: ProgressReporter | None) -> None:
        files = [path for path in staging.rglob("*") if path.is_file()]
        if reporter is not None:
            reporter.files(ProgressStage.INSTALLING_MOD_LOADER, "Applying FTB modpack files...", 0, len(files))
        for index, source in enumerate(files, start=1):
            ...
            if reporter is not None:
                reporter.files(ProgressStage.INSTALLING_MOD_LOADER, f"Applying {relative.as_posix()}...", index, len(files))
