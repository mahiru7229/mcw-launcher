...
class ModrinthController(BaseController):
    ...
    def search(self, project_type: str, query: str, index: str, offset: int, game_version: str = "", loader: str = ModLoaderManager.FABRIC) -> None:
        ...
        task = lambda: (project_type, normalized_loader, ModrinthClient.search_projects(project_type=project_type, query=query, game_version=game_version, loader=normalized_loader, index=index, offset=offset, limit=25, force_refresh=True))
        self._run_network_task(self._task_runner, task_id, task, tr("task.modrinth.search", project_type=project_type, loader=normalized_loader.title()), blocking=False)
    ...
    def load_versions(self, project_type: str, project_id: str, game_version: str = "", loader: str = ModLoaderManager.FABRIC) -> None:
        ...
        task = lambda: (project_type, project_id, normalized_loader, ModrinthClient.list_project_versions(project_id, loader=normalized_loader, game_version=game_version, version_types=("release", "beta", "alpha")))
        self._run_network_task(self._task_runner, task_id, task, tr("task.modrinth.load_versions", loader=normalized_loader.title()), blocking=False)
    ...
    def load_project_details(self, project_type: str, project_id: str, loader: str = ModLoaderManager.FABRIC) -> None:
        ...
        task = lambda: (project_type, project_id, normalized_loader, ModrinthClient.get_project(project_id))
        self._run_network_task(self._task_runner, task_id, task, tr("task.modrinth.load_project_details"), blocking=False)
    ...
    def _on_task_failed(self, task_id: str, error: Exception) -> None:
        ...
        if task_id == "modrinth.install.mod":
            ...
            title = tr("modrinth.modpack.install")
        else:
            title = tr("modrinth.title")
