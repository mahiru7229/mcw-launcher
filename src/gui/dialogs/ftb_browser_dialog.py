...
class FTBBrowserDialog(QDialog):
    ...
    project_details_requested = Signal(int)
    versions_requested = Signal(int, object)
    version_details_requested = Signal(int, int)
    ...
    def __init__(self, parent=None) -> None:
        ...
        self.setObjectName("FTBDialog")
        self._result: FTBSearchResult | None = None
        self._projects: list[FTBProject] = []
        self._selected_project: FTBProject | None = None
        self._versions: list[FTBVersionSummary] = []
        self._selected_version: FTBVersion | None = None
        self._index = 0
        self._busy = False
    ...
    def set_searching(self) -> None:
        ...
        self.result_count_label.setText(tr("ftb.results.searching"))
        self.previous_button.setEnabled(False)
        self.next_button.setEnabled(False)
        self._clear_selection(tr("ftb.results.contacting"))

    def set_search_result(self, result: FTBSearchResult) -> None:
        self._result = result
        ...
        self.result_count_label.setText(tr("ftb.results.range", start=start, end=end, total=result.total_count))
        ...
        if self._projects:
            ...
            self._clear_selection(tr("ftb.results.empty"))

    def set_project_details(self, project_id: int, project: FTBProject) -> None:
    ...
    def set_versions(self, project_id: int, versions: object) -> None:
        ...
        for version in self._versions:
            runtime = " · ".join(value for value in (version.minecraft_version, version.loader.title() if version.loader else "") if value)
            self.version_combo.addItem(f"{version.name} • {version.release_type.title()} • {runtime}", version.version_id)
        ...
        if self._versions:
            ...
            self.detail_panel.set_status(tr("ftb.versions.none", channels=channels))

    def set_version_details(self, project_id: int, version_id: int, version: FTBVersion) -> None:
        if self._selected_project is None or self._selected_project.project_id != int(project_id):
            return
        if int(self.version_combo.currentData() or 0) != int(version_id):
            return
    ...
    def set_cache_info(self, info: FTBCacheInfo) -> None:
        if isinstance(info, FTBCacheInfo):
        ...
        source = tr("ftb.cache.source.cached") if self._cache_info.from_cache else tr("ftb.cache.source.live")
        if not self._cache_info.refreshed_at:
            text = tr("ftb.cache.never", size=f"{size_mb:.1f}", limit=f"{limit_mb:.0f}")
        else:
            text = tr("ftb.cache.status", source=source, size=f"{size_mb:.1f}", limit=f"{limit_mb:.0f}")
        if self._cache_info.last_error:
            text += " · " + tr("ftb.cache.last_error")
    ...
    def set_busy(self, busy: bool) -> None:
        ...
        self.install_button.setEnabled(not self._busy and self._selected_version is not None)
    ...
    def _apply_queued_channel_change(self) -> None:
        ...
        self.release_channel_label.setText(tr("ftb.channel.release_always"))
        if self._selected_project is not None:
            self.versions_requested.emit(self._selected_project.project_id, self.allowed_release_types)
        if not self._busy:
    ...
    def _project_selected(self) -> None:
        ...
        if isinstance(project, FTBProject):
            self._select_project(project)

    def _select_project(self, project: FTBProject) -> None:
        ...
        self.detail_panel.set_loading(
            ...
            project.icon_url,
            project.website_url,
            tr("ftb.project.loading_versions", name=project.name),
        )
        self.project_details_requested.emit(project.project_id)
        self.versions_requested.emit(project.project_id, self.allowed_release_types)
    ...
    def _version_selected(self, _index: int = -1) -> None:
        ...
        version_id = int(self.version_combo.currentData() or 0)
        self.install_button.setEnabled(False)
        if project is None or version_id <= 0:
            return
        self.detail_panel.set_status(tr("ftb.version.loading"))
        self.version_details_requested.emit(project.project_id, version_id)

    def _render_project_details(self, project: FTBProject) -> None:
        game_versions = tuple(dict.fromkeys(version.minecraft_version for version in project.versions if version.minecraft_version))
        loaders = tuple(dict.fromkeys(version.loader.title() for version in project.versions if version.loader))
        metadata = {
            tr("content.metadata.downloads"): f"{project.plays:,}",
            ...
            tr("content.metadata.loaders"): loaders,
            tr("content.metadata.categories"): project.tags[:8],
            tr("ftb.metadata.versions"): len(project.versions),
        }
    ...
    def _apply_row_icon(self, row: int, project_id: int, pixmap: QPixmap) -> None:
        ...
        project = item.data(Qt.ItemDataRole.UserRole) if item is not None else None
        if isinstance(project, FTBProject) and project.project_id == project_id:
    ...
    def _request_install(self) -> None:
        ...
        request = (project.project_id, version.version_id, self.instance_name_input.text().strip(), self.optional_checkbox.isChecked(), self.allowed_release_types)
    ...
    def _safe_instance_name(value: str) -> str:
        ...
        return cleaned[:80] or "FTB Modpack"
    ...
    def retranslate_dynamic(self) -> None:
        self.setWindowTitle(tr("ftb.modpack.title"))
        self.title_label.setText(tr("ftb.modpack.title"))
        self.context_label.setText(tr("ftb.modpack.context"))
        self.search_input.setPlaceholderText(tr("ftb.search.placeholder"))
        self.search_button.setText(tr("common.search"))
        self.refresh_button.setText(tr("ftb.cache.refresh"))
        self.clear_cache_button.setText(tr("ftb.cache.clear"))
        self.release_channel_label.setText(tr("ftb.channel.release_always"))
        self.include_beta_checkbox.setText(tr("ftb.channel.beta"))
        self.include_alpha_checkbox.setText(tr("ftb.channel.alpha"))
        ...
        self.instance_name_input.setPlaceholderText(tr("ftb.modpack.instance_name"))
        self.optional_checkbox.setText(tr("ftb.modpack.optional_files"))
        self.install_button.setText(tr("ftb.modpack.install"))
        self.sort_combo.setItemText(0, tr("ftb.sort.popularity"))
        self.sort_combo.setItemText(1, tr("ftb.sort.updated"))
        self.sort_combo.setItemText(2, tr("ftb.sort.newest"))
        self.sort_combo.setItemText(3, tr("ftb.sort.name"))
        self.results_table.setHorizontalHeaderLabels([
            tr("ftb.column.name"),
            tr("ftb.column.author"),
            tr("ftb.column.installs"),
            tr("ftb.column.updated"),
            tr("ftb.column.description"),
        ])
        if self._result is None:
            self.result_count_label.setText(tr("ftb.results.ready"))
        self.set_cache_info(self._cache_info)
