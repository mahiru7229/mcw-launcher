...
`MCW Update Bridge v1.0.0` breaks that bootstrap loop with a separate recovery executable.
...
2. Download `MCW-Update-Bridge-v1.0.0-windows-x64.exe` from the official repository/release.
3. Run the bridge.
4. Select the folder containing `MCW Launcher.exe` if it is not detected.
5. Click **Update to v1.5.1-beta.2**.
