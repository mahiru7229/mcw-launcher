...
```powershell
python tools/build_release_zip.py --exe ".\dist\MCW Launcher.exe" --version "0.5.0-beta.7"
```
...
MCW-Launcher-v0.5.0-beta.7-windows-x64.zip.sha256
```

The package contains a single wrapper directory:

```text
MCW-Launcher-v0.5.0-beta.7-windows-x64/
├── MCW Launcher.exe
├── mcw-update.json
├── lang/
├── themes/
