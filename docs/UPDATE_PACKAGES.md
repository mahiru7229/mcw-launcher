...
`v1.5.0` predates this architecture and contains the updater bootstrap bug. Existing affected Windows/Linux installs should use `MCW Update Bridge v1.6.0` once to migrate directly to stable v1.5.1. The Bridge verifies the v1.5.1 schema-2 package and installs it without depending on v1.5.0 updater code.

## Required live tests

1. `v1.5.0 → Bridge v1.6.0 → v1.5.1`.
2. `v1.5.1-beta.3+ → v1.5.1` using the bundled automatic updater.
