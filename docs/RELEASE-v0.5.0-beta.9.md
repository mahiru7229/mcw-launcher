...
Beta 9 gộp hai mốc roadmap Accounts/Java và Modpack/Backup thành một bản cập nhật lớn. Microsoft Authentication hiện đã khả dụng, hỗ trợ nhiều tài khoản và có thể hủy đăng nhập mà không làm treo launcher.
...
- Hoàn thiện luồng:
  ...
  - tạo và lưu account.
- Hỗ trợ thêm nhiều tài khoản Microsoft.
- Luôn hiện account picker khi thêm tài khoản mới.
- Nhận diện account bằng Minecraft UUID, không tạo duplicate khi username thay đổi.
- Chạy đăng nhập ở task nền để GUI không bị khóa.
- Có nút **Hủy đăng nhập Microsoft** khi đang chờ callback.
- Đóng browser không còn làm launcher bị treo; người dùng có thể hủy và thử lại ngay.
...
- Completed the authentication pipeline:
  - OAuth PKCE;
  - token refresh;
  - Xbox Live;
  - XSTS;
...
- Added support for multiple Microsoft accounts.
- Always show the account picker when adding another account.
- Identify accounts by Minecraft UUID and avoid duplicates after username changes.
- Run sign-in outside the GUI thread.
- Added **Cancel Microsoft sign-in** while waiting for the OAuth callback.
- Closing the browser no longer freezes the launcher; the flow can be cancelled and retried immediately.
