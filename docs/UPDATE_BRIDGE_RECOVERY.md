# MCW Launcher recovery bridge

## Purpose

The Bridge is a standalone recovery executable for **Windows x64 and Linux x64**. Its default target is **the latest stable release** (`latest`).

It performs a safe migration or recovery without executing updater code from the installed launcher:
...
3. Query the official GitHub release metadata for the latest stable release (or a specified tag).
...
## Bundled updater architecture

The bridge is designed as a standalone recovery tool. For routine updates, each release package carries its own dedicated updater built from the **new release**:
...
The installed launcher downloads/extracts the new package and executes the bundled updater from that package.
